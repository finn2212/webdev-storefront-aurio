<template>
  <div class="sw-side-cart">
    <SfSidebar :title="$t('My cart')" position="right" :visible="isSidebarOpen" :heading-title="$t('My cart')"
      class="sf-sidebar--right" @close="toggleSidebar(false)">
      <template v-if="count" #content-top>
        <SfProperty class="my-cart__total-items" :name="$t('Total items')" :value="count" />
      </template>
      <transition name="fade" mode="out-in">
        <div v-if="count" key="my-cart" class="my-cart">
          <div class="collected-product-list">
            ...
            <transition-group name="fade" tag="div">
              <SwCartProduct v-for="product in cartItems" :key="product.id" :product="product"
        ...
        <div v-else key="empty-cart" class="empty-cart">
          <div class="empty-cart__banner">
            <SwImage :alt="$t('Empty bag')" class="empty-cart__image"
              :src="require('@storefront-ui/shared/icons/empty_cart.svg')" />
            <SfHeading :title="$t('Your cart is empty')" :level="2" class="empty-cart__heading"
      ...
      <template #content-bottom>
        <transition name="fade">
          <div v-if="count">
            <SfProperty :name="$t('Total price')" class="sf-property--full-width sf-property--large my-cart__total-price">
              <template #value>
